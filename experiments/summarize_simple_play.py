...
def main():
    build=read(BASE/'build-report.json');apk=ROOT/'artifacts/pinout-lab.apk'
    assert sha(apk)==build['sha256'] and sha(ROOT/'incoming/pinout/pinout.apk')==build['input_sha256']
    ...
    with zipfile.ZipFile(ROOT/'incoming/pinout/pinout.apk') as original,zipfile.ZipFile(apk) as modified:
