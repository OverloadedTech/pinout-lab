...
def resume():
    ...
    def disk(label):
        data = adb(
            "exec-out",
            "run-as",
            "com.mediocre.pinout.dev",
            "cat",
            "files/quicksave.dat",
        )
        (OUT / (label + ".dat")).write_bytes(data)
        root = decode(data, ROOT / "incoming/pinout/pinout.apk")
        result = {
            "root": root.tag,
    ...
    shell(
        [
            ...
            "start",
            "-n",
            "com.mediocre.pinout.dev/com.mediocre.pinout.MainActivity",
        ]
    )
    warm = save(
        "resume_04_warm",
    ...
    print(
        adb(
            "pull",
            "/data/data/com.mediocre.pinout.dev/files/shdev/events.jsonl",
            OUT / "native-events-before-cold.jsonl",
            timeout=300,
        ).decode(),
        flush=True,
    )
    shell(["am", "force-stop", "com.mediocre.pinout.dev"])
    shell(
        [
            "am",
            "start",
