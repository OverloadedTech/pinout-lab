...
PACKAGE='com.mediocre.pinout.dev'
PORT=18765
...
def forward():
    return adb('forward','tcp:'+str(PORT),'localabstract:pinout_lab').decode()
...
def snapshot():
    ...
    except (OSError,ValueError):return json.loads(adb('exec-out','run-as',PACKAGE,'cat','files/mediocre-lab/snapshot.json'))
...
def pull_log(name='lab-events.jsonl'):
    ...
    with p.open('wb') as output:
        for suffix in ['.2','.1','']:
            try:data=adb('exec-out','run-as',PACKAGE,'cat','files/mediocre-lab/events.jsonl'+suffix,timeout=60)
