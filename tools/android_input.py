...
def start(restart=False):
    adb('forward', 'tcp:' + str(PORT), 'localabstract:pinout_input_lab')
    try:
        running = send('info')
        if not restart:
            return running
    ...
    with (ROOT / 'analysis/reports/input-driver.log').open('w') as log:
        launch = 'CLASSPATH=/data/local/tmp/shdev-input.jar app_process /system/bin dev.pinout.testing.DevInput'
