...
This shared historical page describes Smash Hit first; Granny Smith and PinOut have their own scope at the end. For this repository, start with [PinOut research](pinout.md) and [current verification scope](VERIFICATION.md). The statements below retain the scope of their original reports.
...
Expanded feature verification is tracked separately in notes that are not part of this repository. The desktop GLB contains static baked geometry, not a complete scripted/physical game. Catalogue stitching is an artificial sequence, not a canonical generated campaign. The public bundle contains tools/docs and requires a separately supplied APK. Original tooling and notes now use the [MIT license](../LICENSE).

The Play/Edit release uses native-timestep player movement and separate paused editing semantics, and is documented outside this repository. A native PC port remains a separate deferred phase.
