...
> Historical Smash Hit release report. For PinOut, see [developer controls](developer_mode.md) and [verification scope](VERIFICATION.md).

This page records the archived release named below. Saved editing is documented in [the editor](editor.md); the current Android interface is documented in the Play/Edit notes, which are not part of this repository.
...
**PASS, current Travel release regression, 34 editor checks:** 8 native Android, 7 actual Android UI, 11 desktop browser and 8 desktop-to-Android checks. Native/UI/bridge runs used PID 14443 and APK SHA-256 `1cfe846a63d3a46daf577d045ece235267a822b26ab882a2ce80822c6af31f62`. This includes the added desktop FOV test. Reports are under `experiments/travel_tools/editor_regression/`, with the aggregate at `artifacts/editor-validation-report.json`. These 34 checks are also part of the 55-check Travel release, whose own notes are not part of this repository. Native broad selection covered 505 objects; the separately generated UI room contained 463, and the desktop bridge applied 182 source-box edits with one native undo.
