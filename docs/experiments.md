...
> Historical Smash Hit results. For PinOut, see [game Lab experiments](game_lab_experiments.md) and [verification scope](VERIFICATION.md).

This is the **archived first-release report**. Its APK is retained at `artifacts/releases/phase1/smash-hit-lab.apk`. The second-phase measurements and the current interface and APK verification are recorded in notes that are not part of this repository; the main artifact path points to the current build. Each report is evidence for its recorded APK, not automatically for later builds.
