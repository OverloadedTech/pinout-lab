...
Smash Hit retains its existing Play/Edit interface, documented outside this
repository.
...
See the [measured algorithm](pinout.md#streaming-algorithm).
...
After the [Linux toolchain setup](public_repository.md), provide the original
at the private input path and run:
...
This repository carries the PinOut Lab adapter only. The other addon is built
the same way from its own repository, which ships its own adapter sources.
