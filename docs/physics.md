...
> Shared Smash Hit reference. For PinOut physics and collision geometry, see [game research](pinout.md).
...
The inspected fracture pipeline cuts convex shapes, separates disconnected groups and recomputes body mass properties, with separate cosmetic debris and particles. Its static evidence and unresolved details are documented in the glass shattering notes, which are not part of this repository.
...
Projectile retirement and native entity destruction are documented separately in the ball lifecycle notes, which are not part of this repository. The original-controls path retains original cleanup. First-person Play adds a scoped, bounded projectile-lifetime exception so backward and below-corridor shots can stay in the native solver; that document holds the exact limits and evidence.
