...
> Shared Smash Hit reference. For PinOut, see [developer controls](developer_mode.md). [The documentation index](README.md) explains this export.
...
On the corrected x86_64 build, the full homepage showed the normal `(50,4,0)` view after startup. Camera teleport to `(62,12,-10)` left the player and native update count unchanged. The game transition froze at approximately 0.490 and the return transition around its midpoint; the camera moved while the amount and update count stayed fixed. The second-phase experiments are not part of this repository; the raw records are `experiments/phase2/menu_*.json`.
